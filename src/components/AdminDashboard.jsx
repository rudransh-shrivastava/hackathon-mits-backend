...
const AdminDashboard = () => {
    ...
    const [deviceData, setDeviceData] = useState([
        { id: "Dev-001", name: "iPad Air", status: "Active", policyEnforcement: "97%", restrictionsCount: "All", deploymentTime: "17:10:23 PM" },
        { id: "Dev-002", name: "iMac", status: "Active", policyEnforcement: "91%", restrictionsCount: 43, deploymentTime: "11:32:06 PM" },
        { id: "Dev-003", name: "Dell XPS", status: "Active", policyEnforcement: "88%", restrictionsCount: 52, deploymentTime: "12:48:15 PM" },
        { id: "Dev-004", name: "HP Elite", status: "Disabled", policyEnforcement: "-", restrictionsCount: "No details Available", deploymentTime: "0 Day" },
        { id: "Dev-005", name: "iMAC Pro", status: "InActive", policyEnforcement: "92%", restrictionsCount: 37, deploymentTime: "07:55:33 PM" },
        { id: "Dev-006", name: "Surface Pro", status: "Active", policyEnforcement: "87%", restrictionsCount: 24, deploymentTime: "04:20:12 PM" }
    ]);
    ...
    useEffect(() => {
        const fetchDevices = async () => {
            setLoading(true);
            try {
                const response = await axios.get("https://api.example.com/devices"); // Replace with your actual API URL
                setDeviceData(prevData => [...prevData, ...response.data]); // Merge API data with local data
            } catch (error) {
                                ...
                                <table className="min-w-full">
                                    <thead>
                                        <tr className="bg-gray-100">
                                            ...
                                            <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">No. of Restrictions</th>
                                            <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">Deployment Time</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {currentDevices.map((device, index) => (
                                            <tr key={index} className="border-b">
                                                ...
                                                <td className="px-4 py-3">
                                                    <span className={`px-2 py-1 rounded-full text-xs ${device.status === 'Active' ? 'bg-green-100 text-green-600' : device.status === 'InActive' ? 'bg-[#fcf8ce] text-[#d4cc83]' : 'bg-gray-100 text-gray-600'
                                                        }`}>
                                                        {device.status}
                                                    </span>
                                                </td>
                                                    ...
                                                    {device.policyEnforcement !== '-' && (
                                                        <div className="w-full bg-gray-200 rounded-full h-2">
                                                            <div
                                                                ...
                                                                style={{ width: device.policyEnforcement }}
                                                            ></div>
                                                        </div>
                                                    )}
                                                    <div className="text-xs mt-1">{device.policyEnforcement}</div>
                                                </td>
                                                <td className="px-4 py-3">{device.restrictionsCount}</td>
                                                <td className="px-4 py-3">{device.deploymentTime}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
